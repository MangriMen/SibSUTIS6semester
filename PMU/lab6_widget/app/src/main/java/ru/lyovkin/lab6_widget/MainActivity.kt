...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        private const val cbrURL: String = "https://www.cbr.ru/scripts/xml_metall.asp"
        ...
        fun getData(): MutableMap<String, String> {
            ...
            data["Дата"] =
                records.item(records.length - 1).attributes.getNamedItem("Date").textContent
            for (record in records.length - 4 until records.length) {
                val rec = records.item(record)
                data["${metals[rec.attributes.getNamedItem("Code").textContent]}"] =
                    "${rec.childNodes.item(0).textContent}/${rec.childNodes.item(1).textContent}"
            }
            return data
        }
